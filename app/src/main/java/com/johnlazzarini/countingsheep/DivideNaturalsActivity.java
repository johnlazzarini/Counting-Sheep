...
public class DivideNaturalsActivity extends MathActivity implements MathActivityInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        question.randomizeDivisionOperands(question.getMin(), question.getLowMax());
        ...
        final User theUser = new User("Division", "Natural Numbers", timerLength);

        //Initializes the display field
        final TextView prompt = (TextView) findViewById(R.id.prompt_id);
        prompt.setText(question.toDivisionString());
        ...
        createAndActivateTimer(timerLength, contextDisplay, dataDisplay, recipientNameArea,
                inputOutput, progressRoot, sendEmail, submit, DivideNaturalsActivity.this, theUser);
    }

    //overloaded methods
    public void addSheep(LinearLayout root, User theUser){
        ImageView newSheepFace = new ImageView(DivideNaturalsActivity.this, null);
    ...
    public boolean userAnswerIsCorrect(EditText theEntry, RandomQuestion theQuestion){
        int userAnswer = Integer.parseInt(theEntry.getText().toString().trim());
        int correctAnswer = theQuestion.getLeftOperand() / theQuestion.getRightOperand();

        return userAnswer == correctAnswer;
    }

    public void generateNewQuestion(TextView thePrompt, RandomQuestion theQuestion){
        theQuestion.randomizeDivisionOperands(theQuestion.getMin(),theQuestion.getLowMax());
        thePrompt.setText(theQuestion.toDivisionString());
    }
}